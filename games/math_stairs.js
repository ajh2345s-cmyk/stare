module.exports = {
    ...
    handleGameOver: function(store, socketId, data) {
        ...
        if (store.data.mathStairsScores && store.data.mathStairsScores[socketId]) {
            const s = store.data.mathStairsScores[socketId];
            const floor = Math.max(1, Number(data && data.floor) || s.floor || 1);
            const score = Math.max(0, Number(data && data.score) || s.score || 0);
            s.floor = floor;
            s.score = score;
            s.state = 'gameover';
        }
        ...
        // 모든 학생이 끝난 순간 기존 프로젝트의 결과창 흐름으로 종료
        const students = Object.keys(store.players).filter(id => !store.players[id].isAdmin);
        const allDone = students.length > 0 && students.every(id => {
            const s = (store.data.mathStairsScores || {})[id];
            return s && s.state === 'gameover';
        });
        if (allDone) this.endGame(store, store.__mathStairsLogic);
    },
